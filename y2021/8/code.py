...
def part_two(puzzle_input):
    return sum(resolve_output(unique, output) for unique, output in puzzle_input)
...
def resolve_output(unique_signals, output):
    ...
    result = sum(
        next(filter(lambda x: x[1] == encoded_digit, encoding.items()))[0]
        * (10 ** (len(output) - idx - 1))
...
def signal_characteristics(signals, simple_signals):
    return [
        tuple(
            len(signal.intersection(simple_signal)) for simple_signal in simple_signals
        )
        for signal in signals
    ]


default_characteristics = dict(
    zip(
        signal_characteristics(
            digits.values(), [digits[digit] for digit in easy_digits]
        ),
        digits.keys(),
    )
)
...
def resolve_encoding(signals):
    easy_signals = [
        next(signal for signal in signals if len(signal) == len(digits[digit]))
